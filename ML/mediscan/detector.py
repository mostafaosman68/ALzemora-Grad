...
class MedicineDetector:
    ...
    def _match_one(
        ...
        try:
            knn = self._matcher.knnMatch(des_ref, des_frame, k=2)
        except cv2.error:
            return None

        good = [
            m for m, n in (pair for pair in knn if len(pair) == 2)
            if m.distance < RATIO_TEST * n.distance and m.distance < ORB_DISTANCE_THRESHOLD
        ]
