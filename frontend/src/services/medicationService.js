...
export async function updateMedicationSchedule(medicationId, schedule) {
  const response = await fetchWithTimeout(
    `${BASE_URL}/medications/${encodeURIComponent(medicationId)}/schedule`,
    {
      method: 'PATCH',
      headers: {
        Accept: 'application/json',
...
export async function deleteMedication(medicationId) {
  const response = await fetchWithTimeout(
    `${BASE_URL}/medications/${encodeURIComponent(medicationId)}`,
    {
      method: 'DELETE',
      headers: {
        Accept: 'application/json',
