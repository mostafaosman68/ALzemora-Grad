...
# Explicitly-set active patient (overrides last-login heuristic)
_active_patient_id: Optional[str] = None
...
async def login(
    ...
    try:
        ...
        # Record login time so the Raspberry Pi recognizer can identify the active patient
        try:
            if role == "User":
                ...
                await db.users.update_one(
                    {"_id": ObjectId(selected_patient_id)},
                    {"$set": {"last_login_at": datetime.datetime.utcnow()}},
                )
        except Exception:
            pass  # Non-critical — don't fail login if timestamp write fails
...
# ─── Explicitly set the active patient (called on login or patient switch) ────
@router.post("/set-active-patient")
async def set_active_patient(patient_id: str = Body(..., embed=True)):
    """Store which patient is currently active so the multimodal recognizer
    always loads the right person's data regardless of login timestamps."""
    ...
    _active_patient_id = str(patient_id)
    print(f"[ACTIVE PATIENT] Set to: {user.get('full_name')} (ID: {_active_patient_id})")
    return {"status": "ok", "patient_id": _active_patient_id, "full_name": user.get("full_name")}


# ─── Get most recently logged-in patient ──────────────────────
@router.get("/active-patient")
async def get_active_patient():
    """Return the active patient. Prefers the explicitly-set patient (via
    /set-active-patient) so guardian patient-switches are reflected immediately.
    Falls back to the most recently logged-in patient."""
    try:
        db = get_db()
        if db is None:
            return {"error": "Database connection failed"}

        # Use explicitly-set patient if available
        if _active_patient_id:
            try:
                user = await db.users.find_one({"_id": ObjectId(_active_patient_id)})
                if user:
                    print(f"[ACTIVE PATIENT] Returning (explicit): {user.get('full_name')} (ID: {user['_id']})")
                    return {
                        "user_id": str(user["_id"]),
                        "full_name": user.get("full_name"),
                        "email": user.get("email"),
        ...
        # Fall back to the patient with the most recent last_login_at timestamp
        result = await db.users.find(
            {"last_login_at": {"$exists": True}}
        ).sort("last_login_at", -1).limit(1).to_list(length=1)

        if not result:
            result = await db.users.find({}).limit(1).to_list(length=1)

        if not result:
            return {"error": "No patient found in the database"}

        user = result[0]
        print(f"[ACTIVE PATIENT] Returning (last-login): {user.get('full_name')} (ID: {user['_id']})")
        return {
            "user_id": str(user["_id"]),
            "full_name": user.get("full_name"),
            "email": user.get("email"),
        ...
        print(f"[ACTIVE PATIENT ERROR] {str(e)}")
        return {"error": f"Error fetching active patient: {str(e)}"}
