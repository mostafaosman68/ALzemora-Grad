...
def _extract_date_time(schedule) -> tuple:
    """Return (days_list, time_str) from a schedule dict; (None, None) if not parseable."""
    if not isinstance(schedule, dict):
        return None, None
    date = schedule.get("days") or None
    time = (
        schedule.get("time")
        or (
            f"{schedule.get('hour', '08')}:{schedule.get('minute', '00')} {schedule.get('period', 'AM')}"
...
async def add_medication(
    ...
    # Extract date (days array) and time string as explicit top-level fields
    _date, _time = _extract_date_time(parsed_schedule)
...
@router.patch("/{medication_id}/schedule")
async def update_medication_schedule(medication_id: str, request: Request):
    db = get_db()
    if db is None:
        raise HTTPException(status_code=500, detail="Database connection failed")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="medication_id is invalid") from exc
    ...
    if schedule is None:
        raise HTTPException(status_code=400, detail="'schedule' field is required")

    # Ensure we store a plain dict, not any framework wrapper type
    if not isinstance(schedule, (dict, str)):
        raise HTTPException(status_code=400, detail="'schedule' must be an object or string")

    medication = await db.medications.find_one({"_id": med_object_id})
    if not medication:
        raise HTTPException(status_code=404, detail="Medication not found")

    clean_schedule = dict(schedule) if isinstance(schedule, dict) else schedule

    # Merge partial update with the stored schedule so unchanged fields are preserved
    existing_schedule = medication.get("schedule") or {}
    if not isinstance(existing_schedule, dict):
        existing_schedule = {}
    merged_schedule = {**existing_schedule, **clean_schedule}

    _date, _time = _extract_date_time(merged_schedule)

    # Only write the top-level date/time fields that were actually provided
    set_fields: dict = {"schedule": merged_schedule, "updated_at": datetime.utcnow()}
    if "days" in clean_schedule:
        set_fields["date"] = _date
    if clean_schedule.get("time") or clean_schedule.get("hour"):
        set_fields["time"] = _time
    ...
    if result.modified_count == 0:
        raise HTTPException(status_code=500, detail="Schedule was not updated in the database")

    updated = await db.medications.find_one({"_id": med_object_id})
    return {"message": "Schedule updated", "medication": _serialize_medication(updated)}
...
async def delete_medication(medication_id: str):
    ...
    med_folder = MEDICATIONS_DIR / str(patient_id) / _sanitize_name(medication.get("name", ""))
    if med_folder.exists():
        shutil.rmtree(med_folder)

    await db.medications.delete_one({"_id": med_object_id})
